...
Shader "Sprites/ShinyDefault"
{
	Properties
	{
		...
		_worldSpacePosOffSet("World space Y location offset", Range(-1,1)) = 0
		...
		SubShader
	{
		...
		Pass
		{
			...
			float _ShineWidth;
			float _worldSpacePosOffSet;
			float _amount;

			fixed4 SampleSpriteTexture(float2 uv, float4 vert)
			{
				...
				_ShineLocation = _ShineLocation + (vert.y / 1000) + _worldSpacePosOffSet;
				...
				if (currentDistanceProjection > lowLevel && currentDistanceProjection < highLevel) {
					...
					color.rgb += color.a * whitePower / _amount;
				}
			...
			{
				fixed4 c = SampleSpriteTexture(IN.texcoord, IN.vertex) * IN.color;
				c.rgb *= c.a;
